// ========== REALMS&SCRIPTS SYSTEM ==========
// Complete Realms&Scripts (Draunia) implementation

const RealmsScriptsSystem = {
    id: 'realsscripts',
    name: 'Realms&Scripts',
    description: 'Sistema original de Draunia com Fusões (Heart, Brain, Muscle, Bone), PA, PE, Sanidade, 6 atributos e mecânicas únicas de javacons.',
    icon: 'fa-scroll',
    ...
    // ========== ATTRIBUTES ==========
    attributes: [
        { id: 'attrFor', name: 'FOR', fullName: 'Força', description: 'Poder físico bruto, capacidade de carga' },
        { id: 'attrCon', name: 'CON', fullName: 'Constituição', description: 'Resistência física, saúde, vigor' },
        { id: 'attrVon', name: 'VON', fullName: 'Vontade', description: 'Força mental, determinação, PE' },
        { id: 'attrCar', name: 'CAR', fullName: 'Carisma', description: 'Influência social, liderança' },
        { id: 'attrInt', name: 'INT', fullName: 'Inteligência', description: 'Conhecimento, raciocínio, perícias' },
        { id: 'attrAgi', name: 'AGI', fullName: 'Agilidade', description: 'Velocidade, reflexos, PA' }
    ],
    ...
    data: {
        ...
        armorBonus: {
            'none': { ca: 0, weight: 0, skillPenalty: 0, paPenalty: 0, name: 'Sem Armadura' },
            'light': { ca: 3, weight: 3, skillPenalty: 0, paPenalty: 0, name: 'Leve' },
            'medium': { ca: 5, weight: 5, skillPenalty: -2, paPenalty: -1, name: 'Média' },
            'heavy': { ca: 7, weight: 7, skillPenalty: -5, paPenalty: -3, name: 'Pesada' }
        },
        
        // Fusion types
        fusions: {
            heart: { name: 'Coração', effect: 'HP máximo' },
            brain: { name: 'Cérebro', effect: 'PE máximo' },
            muscleArms: { name: 'Músculos (Braços)', effect: 'Bônus em Luta' },
            muscleHead: { name: 'Músculos (Cabeça)', effect: 'Bônus em Percepção' },
            muscleBack: { name: 'Músculos (Costas)', effect: 'Bônus em Pontaria' },
            muscleChest: { name: 'Músculos (Peito)', effect: 'Bônus em Atletismo' },
            muscleLegs: { name: 'Músculos (Pernas)', effect: 'Bônus em Acrobacia' },
            boneArms: { name: 'Ossos (Braços)', effect: 'Redução de dano ao defender' },
            boneHead: { name: 'Ossos (Cabeça)', effect: 'Redução de dano geral' },
            boneBack: { name: 'Ossos (Costas)', effect: 'Redução de dano geral' },
            boneChest: { name: 'Ossos (Peito)', effect: 'Redução de dano geral' },
            boneLegs: { name: 'Ossos (Pernas)', effect: 'Redução de dano de queda' }
        }
    },
    ...
    calculations: {
        ...
        getAttrValue(charData, attrName) {
            const attrMap = {
                'FOR': 'attrFor', 'for': 'attrFor', 'STR': 'attrFor', 'str': 'attrFor',
                'CON': 'attrCon', 'con': 'attrCon',
                'VON': 'attrVon', 'von': 'attrVon', 'WIS': 'attrVon', 'wis': 'attrVon',
                'CAR': 'attrCar', 'car': 'attrCar', 'CHA': 'attrCar', 'cha': 'attrCar',
                'INT': 'attrInt', 'int': 'attrInt',
                'AGI': 'attrAgi', 'agi': 'attrAgi', 'DEX': 'attrAgi', 'dex': 'attrAgi'
            };
        ...
        /**
         * Calculate Max HP
         * Formula: (heartLevel × 20) + (level × CON)
         */
        ...
        /**
         * Calculate Max PE
         * Formula: (brainLevel × 6) + (level × VON)
         */
        ...
        /**
         * Calculate PA (Pontos de Ação)
         * Formula: AGI + Level + 4 - armor penalty - weight penalty
         */
        calculatePA(charData) {
            const agi = parseInt(charData.attrAgi) || 0;
            const level = parseInt(charData.charLevel) || 1;
            const armorPenalty = RealmsScriptsSystem.data.armorBonus[charData.armorType || 'none'].paPenalty;
        ...
        /**
         * Calculate CA
         * Formula: 10 + armor bonus
         */
        calculateCA(charData) {
            const armorBonus = RealmsScriptsSystem.data.armorBonus[charData.armorType || 'none'].ca;
            return 10 + armorBonus;
        },
        
        /**
         * Calculate Dodge
         * Formula: CA + Reflexos skill total
         */
        ...
        calculateCurrentWeight(charData) {
            let weight = RealmsScriptsSystem.data.armorBonus[charData.armorType || 'none'].weight;
        ...
        calculateSkillTotal(charData, skillId) {
            const skill = RealmsScriptsSystem.skills.find(s => s.id === skillId);
            if (!skill) return 0;
            ...
            total += RealmsScriptsSystem.data.trainingLevels[trainingLevel]?.bonus || 0;
            
            // Armor penalty
            if (skill.armorPenalty) {
                const armorType = charData.armorType || 'none';
                total += RealmsScriptsSystem.data.armorBonus[armorType].skillPenalty;
            }
...
if (typeof SystemRegistry !== 'undefined') {
    SystemRegistry.register('realsscripts', RealmsScriptsSystem);
}

// Export for ES modules
if (typeof module !== 'undefined' && module.exports) {
    module.exports = RealmsScriptsSystem;
}
